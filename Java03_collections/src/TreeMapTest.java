...
public class TreeMapTest {
	...
	public TreeMapTest() {
		// TreeMap : key, value를 가진다. key기준으로 정렬 
		...
		Member m = ht.get("이순신");
		m.memberPrn();
		System.out.println("-----------------------keySet 으로 목록구하기 ascending-------");
		 ...
		 Collection<Member> valueSet = ht.values();
		 Iterator<Member> iiii = valueSet.iterator();
		 while(iiii.hasNext()) {
			 Member mm = iiii.next();
			 mm.memberPrn();
		 }
